...
def update_plabels(opt, support, support_ys, query):
    ...
    X = np.concatenate((support, query), axis=0).copy(
        order='C')
    k = opt.K
    ...
    p_labels = np.argmax(probs_l1, 1)  # 公式11
...
def get_projection_matrix(features, labels, proj_dim):
    ...
    B = fix_numerical_assymetry(np.matmul(np.matmul(X, L), X.transpose()) + 0 * np.eye(d))
...
def iter_balanced_trans(opt, support_features, support_ys, query_features, query_ys, labelled_samples):
    ...
    # 1 l2 normalization and pca
    support_features, query_features = do_l2_normalization(support_features, query_features)

    support_features, query_features = do_pca(support_features, query_features, 5)
    # support_features = torch.tensor(support_features)
    support_features, query_features = do_l2_normalization(support_features, query_features)
    # 2 initial
    feats_all = np.concatenate((support_features, query_features), axis=0)

    query_ys_pred, _, _ = update_plabels(opt, support_features, support_ys, query_features)

    for t in range(1, 11):
        P = get_projection_matrix(feats_all, np.concatenate((support_ys, query_ys_pred), axis=0), 5)
        support_features, query_features = project_features(P, support_features), project_features(P, query_features)
        support_features, query_features = center_and_l2_normalize(support_features, query_features)
        ...
        query_ys_pred, query_features_pred = get_closed_set_pseudo_labels(support_features, support_ys, query_features)
        selected = select_closed_set_pseudo_labels(query_ys_pred, query_features_pred, t, 11)  # 已选为1，未选为0
...
if __name__ == '__main__':
    ...
    dataset = params.dataset
    print(ndatas.shape)  # torch.Size([1000, 5, 16, 640])
